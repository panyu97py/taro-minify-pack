...
Component({ lifetimes: { attached: () => SingletonPromise.resolve() } })
`
...
export class InjectStyleComponentPlugin {
  ...
  public static generateComponentPath (opt:Opt) {
  ...
  apply (compiler: Compiler) {
    compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation: Compilation) => {
      ...
      compilation.hooks.processAssets.tap({ name: PLUGIN_NAME, stage }, (assets) => {
        const dynamicPackageStyleFileRegex = new RegExp(`^${this.opt.dynamicPackageName}\\/.*\\.wxss$`)

        const styleFileContent = Object.keys(assets).reduce((result, assetPath) => {
          if (!dynamicPackageStyleFileRegex.test(assetPath)) return result
          const relativePath = path.relative(this.opt.dynamicPackageName, assetPath)
